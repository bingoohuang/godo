...
export MYSQL_EXEC="%host MYSQL_PWD='8BE4-62A56623CA8c' mysql -uroot -h127.0.0.1 -P9633 -vvv -e "
...
"
    ...
    echo "${now} 查看从状态"
    ${gossh} --replaceQuote=%q --cmds="${MYSQL_EXEC} %qshow slave status%q"
elif ((godoNum == 4)); then
    echo "${now} 查看各个库中表的数据行数"
    ${gossh} --separator=%s --replaceQuote=%q --cmds="${MYSQL_EXEC} %q \
...
%q"
    ...
    echo "${now} 插入1000条数据"
    ${pump}  -b 100 -r 1000 -t a.ta,b.tb,c.tc,d.td,e.te
fi
